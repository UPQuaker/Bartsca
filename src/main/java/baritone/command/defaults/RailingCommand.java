...
public class RailingCommand extends Command {

    public RailingCommand(IBaritone baritone) {
        super(baritone, "railing");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (args.hasExactly(3)) {
            ...
            if (width < 1 || height < 1 || depth < 1 || height > 255) {
                logDirect("Width and depth must at least be 1 block; Height must at least be 1 blocks, and cannot be greater than the build limit.");
                cont = false;
            }
            ...
            if (cont) {
                ...
                logDirect(String.format("Creating a railing %s block(s) high, %s block(s) wide, and %s block(s) deep", height + 1, width + 1, depth));
                baritone.getBuilderProcess().railArea(corner1, corner2);
            }
        } else {
    ...
    public String getShortDesc() {
        return "Set a goal to rail in your current direction";
    }

    @Override
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The railing command sets a goal that tells Baritone to place obsidian completely straight in the direction that you're facing.",
                "",
                "Usage:",
